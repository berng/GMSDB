    ...
    for i in range(gm.n_components):
        for j in range(gm.n_components):
            ...
            if show:
                ...
                axs[0].hist(pd_ii[np.where(np.isnan(pd_ii), False, True)], bins=100, histtype='step', density=True,
                            label='ii')
                axs[0].hist(pd_ij[np.where(np.isnan(pd_ij), False, True)], bins=100, histtype='step', density=True,
                            label='ij')
                axs[0].hist(pd_ji[np.where(np.isnan(pd_ji), False, True)], bins=100, histtype='step', density=True,
                            label='ji')
                axs[0].hist(pd_jj[np.where(np.isnan(pd_jj), False, True)], bins=100, histtype='step', density=True,
                            label='jj')
                axs[0].legend()
...
def ConfiDistance(MatrixClustersAll, gm=False, eps=1., show=False, alpha=0.1, bh_mh_correct=False, dim=2):
    """ Joins clusters into super clusters using DBSCAN and calculates MC quality index
...
class GMSDB:
    def __init__(self, min_components=2, step_components=1, n_components=2, verbose=False, show=False, metric='LH',
                 border_percentile=0.001, alpha_stage2=0.05, alpha_stage4=0.1, show_mah=False, show_clusters=False,
                 autostop=True, bh_mh_correct=False, rand_search=0, rand_level=0.5, max_iter=200, centroid_type='mean'):
        """ Init class
    ...
    @param centroid_type define method for centroid calculation. Default mean (median, geometric, rms)
  ...
  """
        ...
        # centroids
        self.centroid_type: str = centroid_type
    ...
    def getOptSuperclusters(self, Matrix, gm, X, Y, eps_np_mid):
        ...
        if self.rand_search > 0:
            ...
            for eps_itt in search_idx[:self.rand_search]:
                ...
                cdist, labels = ConfiDistance(MatrixTmp, self.gm, eps=eps, show=self.verbose,
                                              alpha=self.alpha_stage4, bh_mh_correct=self.bh_mh_correct,
                                              dim=self.MH_dim)
                if 1 >= cdist >= 0.:
        ...
        for eps_itt in search_idx:
            ...
            for subitt in range(self.SUBITT):
                ...
                if self.verbose:
                    print('check', eps_itt, '/', eps_np_mid.shape[0], ' eps:', eps, 'cdist', cdist, 'labels:',
                          len(set(list(labels))), 'eps lim:', eps_supermax, eps_supermin)
                ...
                if cdist >= 1.0 and self.autostop:
                    if self.verbose:
                        print('============== no more itterations needed - absolute optimum reached! ==========',
                              flush=True)
                    return self.best_cluster_num, self.labels
        ...
        if self.verbose:
            print('============== Unpredicted return ! ==========', cdist_opt, self.best_cluster_num, self.labels,
                  eps_supermax, eps_supermin, flush=True)
        return self.best_cluster_num, self.labels

    def fit(self, X0, show_closest=False, plot_decisions=False, max_bic_search_size=-1):
        """ Train model
        @param X0:
        @param show_closest: Show two clusters closest to give. Debug option
        @param plot_decisions: Plot decision regions when fitting. Debug option
        @param max_bic_search_size: If >0 uses for bic search stage (stage1) shorter dataset to provide faster search. Debug option
        """
    ...
    def _calculate_centroids(self, X):
        """Calculates centroids for each cluster based on the selected method.
        @param X: Train dataset
        """
        unique_labels = np.unique(self.yp)
        centroids = np.zeros((len(unique_labels), X.shape[1]))

        for i, label in enumerate(unique_labels):
            points_in_cluster = X[self.yp == label]

            if self.centroid_type == 'mean':
                centroid = np.mean(points_in_cluster, axis=0).reshape(1, -1)
            elif self.centroid_type == 'median':
                centroid = np.median(points_in_cluster, axis=0).reshape(1, -1)
            elif self.centroid_type == 'geometric':
                centroid = ss.gmean(points_in_cluster, axis=0).reshape(1, -1)
            elif self.centroid_type == 'rms':
                centroid = np.sqrt(np.mean(np.square(points_in_cluster), axis=0)).reshape(1, -1)
            else:
                raise ValueError(f"Unknown centroid calculation method: {self.centroid_type}")

            centroids[i] = self.ss.inverse_transform(centroid)[0]
